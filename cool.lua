...
-- Remotes tab (unchanged - lazy load)
...
local states = {
    ESP = false,
    BoxESP = false,
    Aimbot = false,              -- forced OFF at start
...
local aimTeamMode = "Enemies"   -- "Enemies", "Teammates", "Everyone"
...
local function getClosestTarget()
    ...
    for _, plr in Players:GetPlayers() do
        ...
        if aimTeamMode == "Enemies" and sameTeam then continue end
        if aimTeamMode == "Teammates" and not sameTeam then continue end
        -- "Everyone" → allow all (no continue)
...
local function toggleAimbot(on)
    if on then
        setStatus("Aimbot ON → " .. aimTeamMode .. (states.AimbotThroughWalls and " (through walls)" or ""))
...
-- Aimbot section with team selector
...
teamFrame.Size = UDim2.new(1, 0, 0, 50)
teamFrame.BackgroundColor3 = Color3.fromRGB(32, 32, 52)
teamFrame.Parent = aimbotSub
Instance.new("UICorner", teamFrame).CornerRadius = UDim.new(0, 10)

local teamLabel = Instance.new("TextLabel", teamFrame)
teamLabel.Size = UDim2.new(0.38, 0, 1, 0)
teamLabel.BackgroundTransparency = 1
teamLabel.Text = "Aim at:"
teamLabel.TextColor3 = Color3.fromRGB(200, 220, 255)
teamLabel.Font = Enum.Font.GothamSemibold
teamLabel.TextSize = 16
teamLabel.TextXAlignment = Enum.TextXAlignment.Left
teamLabel.Position = UDim2.new(0.03, 0, 0, 0)

local teamOptions = {"Enemies", "Teammates", "Everyone"}
for i, mode in ipairs(teamOptions) do
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0.19, 0, 0.76, 0)
    btn.Position = UDim2.new(0.42 + (i-1)*0.205, 0, 0.12, 0)
    btn.BackgroundColor3 = (aimTeamMode == mode) and Color3.fromRGB(80, 180, 255) or Color3.fromRGB(55, 55, 75)
    btn.Text = mode
    btn.TextColor3 = Color3.new(1,1,1)
    btn.Font = Enum.Font.Gotham
    btn.TextSize = 14
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
    btn.Parent = teamFrame
    
    btn.MouseButton1Click:Connect(function()
        aimTeamMode = mode
        for _, b in ipairs(teamFrame:GetChildren()) do
            if b:IsA("TextButton") then
                b.BackgroundColor3 = (b.Text == mode) and Color3.fromRGB(80, 180, 255) or Color3.fromRGB(55, 55, 75)
            end
        end
        setStatus("Aimbot target: " .. mode)
        if states.Aimbot then
            toggleAimbot(false)
            task.wait()
            toggleAimbot(true)
...
task.defer(function()
    ...
    print("Diddy Cheats V6.7 loaded successfully (with team aimbot)")
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.LeftAlt then
        ...
        setStatus("Aimbot " .. (newState and "ON" or "OFF") .. " ← LeftAlt (" .. aimTeamMode .. ")")
    end
end)
